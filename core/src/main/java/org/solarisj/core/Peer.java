...
import org.solarisj.net.StreamConnection;
import org.solarisj.store.BlockStore;
import org.solarisj.store.BlockStoreException;
import org.solarisj.utils.ListenerRegistration;
 ...
 * {@link org.solarisj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.solarisj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.solarisj.net.NioClientManager#openConnection(java.net.SocketAddress, StreamConnection)}
     * or
     * {@link org.solarisj.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.</p>
     *
    ...
    /**
     * <p>Construct a peer that reads/writes from the given block chain. Transactions stored in a {@link org.solarisj.core.TxConfidenceTable}
...
/*
        if (m instanceof VersionMessage){
            ...
            System.out.println("not VersionMessage\n ");
        }
        if (m instanceof VersionAck){
            System.out.println("VersionAck\n ");
        } else {
            System.out.println("not VersionAck\n ");
        }

        if(versionHandshakeFuture.isDone()){
            System.out.println("Done\n ");
        } else {
            System.out.println("not Done\n ");
        }
        if(versionHandshakeFuture.isCancelled()){
...
*/
        ...
        else if(m instanceof SporkMessage)
        {
            ...
            // todo furszy: check solaris masternodes
    ...
    private void processVersionMessage(VersionMessage m) throws ProtocolException {
        ...
        // solarisj is a client mode implementation. That means there's not much point in us talking to other client
...
//            if(!instantSend.mapTxLockVotes.containsKey(item.hash))
     ...
     * Once the pong is received the value returned by {@link org.solarisj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.solarisj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.solarisj.core.Peer#ping()} has never
