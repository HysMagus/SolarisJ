...
import org.solarisj.core.listeners.BlockChainListener;
import org.solarisj.store.FlatDB;
import org.solarisj.store.HashStore;
import org.solarisj.store.MasternodeDB;
 ...
 * solarisj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating solarisj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use solarisj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a solarisj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using solarisj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing solarisj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.solarisj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.solarisj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.solarisj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initSolaris(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### Solaris init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented solaris MN stuff
