...
import org.solarisj.core.BloomFilter;
import org.solarisj.core.ECKey;
import org.solarisj.core.NetworkParameters;
import org.solarisj.core.Utils;
import org.solarisj.crypto.*;
import org.solarisj.script.Script;
import org.solarisj.utils.Threading;
 ...
 * {@link org.solarisj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link org.solarisj.crypto.MnemonicCode},
 ...
 * {@link org.solarisj.crypto.DeterministicKey#serializePubB58(org.solarisj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link org.solarisj.crypto.DeterministicKey#deserializeB58(org.solarisj.crypto.DeterministicKey, String, org.solarisj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(org.solarisj.crypto.DeterministicKey,KeyChainType keyChaintype)}.</p>
 *
 * <p>This class builds on {@link org.solarisj.crypto.DeterministicHierarchy} and
 * {@link org.solarisj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    public static final ChildNumber SOLARIS_PATH = new ChildNumber(119,true);
    public static final ImmutableList<ChildNumber> BIP44_ACCOUNT_ZERO_PATH =
            ImmutableList.of(BIP44_MASTER_KEY, SOLARIS_PATH, ChildNumber.ZERO_HARDENED);
    ...
    public static class Builder<T extends Builder<T>> {
        ...
        public DeterministicKeyChain build() {
            ...
            if (random != null) {
                ...
                chain = new DeterministicKeyChain(watchingKey,KeyChainType.BIP44_Solaris_ONLY);
            }
    ...
    protected DeterministicKeyChain(DeterministicSeed seed) {
        this(seed, null,KeyChainType.BIP44_Solaris_ONLY);
    }
    ...
    public DeterministicKeyChain(DeterministicKey watchingKey,KeyChainType keyChainType) {
        ...
        if (keyChainType != KeyChainType.BIP44_Solaris_ONLY)
    ...
    protected DeterministicKeyChain(DeterministicKey watchKey, boolean isFollowing) {
        this(watchKey,KeyChainType.BIP44_Solaris_ONLY);
    ...
    public static DeterministicKeyChain watch(DeterministicKey accountKey) {
        return new DeterministicKeyChain(accountKey,KeyChainType.BIP44_Solaris_ONLY);
    }
    ...
    private void initializeHierarchyUnencrypted(DeterministicKey baseKey) {
        if (baseKey.isPubKeyOnly() && keyChainType == KeyChainType.BIP44_Solaris_ONLY){
    ...
    public List<DeterministicKey> getKeys(KeyPurpose purpose, int numberOfKeys) {
        ...
        try {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                ...
                // by any real problem reports from solarisj users, but I've heard of cases via the grapevine of
     ...
     * The returned key can be serialized and then passed into {@link #watch(org.solarisj.crypto.DeterministicKey)}
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter, KeyChainFactory factory) throws UnreadableWalletException {
        ...
        KeyChainType keyChainType = KeyChainType.BIP44_Solaris_ONLY;
        ...
        for (Protos.Key key : keys) {
            ...
            if (!path.isEmpty() && path.get(0).equals(BIP44_MASTER_KEY)){
                // is BIP44
                keyChainType = KeyChainType.BIP44_Solaris_ONLY;
                break;
            }
        }
    ...
    public DeterministicKeyChain toEncrypted(KeyCrypter keyCrypter, KeyParameter aesKey) {
        return toEncrypted(keyCrypter,aesKey,KeyChainType.BIP44_Solaris_ONLY);
    }
     ...
     * @see org.solarisj.wallet.MarriedKeyChain
     */
